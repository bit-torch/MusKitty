...
> 最后更新: 2026-09-13 | **M-3 batch 3（CSS 补全：line-height 精确解析 + text-transform）**：`line-height` 此前是 `font_size * 1.2` 硬编码近似（T-3 遗留），`text-transform` 零消费方。本批新增 cascade `text_props` 模块作为**测量与绘制共用的单一语义来源**（`used_line_height_px`：px/数/百分比/normal + 非法值钳制；`apply_text_transform`：Unicode 全尺寸映射 + capitalize），`line-height: <percentage>` 在计算值阶段转 px、`<number>` 保持倍数形态随继承；layout `NodeContext::Text` 携带行高、文本叶在建树时即应用转换（`InheritedText` 结构体收敛继承参数），renderer `RenderCommand::Text` 新增 `line_height` 且 paint 用同一实现改写内容（`b66d0e4` / `1730e94` / `bdd1dae`）。验证：cascade 225 / layout 121 / renderer 110 / chrome 94 全绿，含"转换后文本与字面量大写测出完全相同排版"的等式断言与全链路像素断言（60px 行高下推末行 >40px、总墨迹差 <10%），fmt/clippy 干净。剩余缺口（letter/word-spacing、font-style、white-space、text-indent、direction）与批次排期见 [docs/plans/2026-09-12-css-completion.md](docs/plans/2026-09-12-css-completion.md)
>
